...
function ES({ data, page, limit, province, search, categories, country, year, tags, sort, context: { setCountry } }) {
  useEffect(() => {
    if (country !== 'ES')
      setCountry('ES')
  }, [])
  
  return <Home data={data} page={page} limit={limit} province={province} search={search} categories={categories} country='ES' year={year} tags={tags} sort={sort} />
}
...
export async function getServerSideProps(context) {
  ...
  let country = getCookie('country', { req, res })
  ...
  try {
    return retrieveFilteredAds('ES', page, limit, province, search, categories, year, tags2, sort)
      .then(data => {
        const arr2 = []
        if (tags) {

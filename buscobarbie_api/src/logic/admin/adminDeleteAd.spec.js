...
describe('Admin delete ad', () => {
    ...
    it('fails with admin user not found', async () => {
        ...
        try {
            await adminDeleteAd(userId, ad.id)
        } catch (error) {
    ...
    it('fails with not authorized user', async () => {
        ...
        try {
            await adminDeleteAd(user.id, ad.id)
        } catch (error) {
    ...
    it('fails with not found ad', async () => {
        ...
        try {
            await adminDeleteAd(user.id, adId)
        } catch (error) {
            expect(error).toBeInstanceOf(NotFoundError)
    ...
    it('succeeds deletting user ad', async () => {
        ...
        try {
            const info = await adminDeleteAd(user.id, ad.id)
            expect(info).toBeInstanceOf(Object)
            expect(info.accepted).toHaveLength(1)
            expect(info.accepted).toEqual([email])
            const ad2 = await Ad.findById(ad.id).lean()
            expect(ad2).toBeNull()
        } catch (error) {
